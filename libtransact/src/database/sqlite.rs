...
impl SqliteDatabaseBuilder {
    ...
    pub fn build(self) -> Result<SqliteDatabase, SqliteDatabaseError> {
        ...
        let prefix = self.prefix.map(|s| s + "_").unwrap_or_else(|| "transact_".into());
...
fn execute_index_count(
    ...
    let query = format!(
        "SELECT COUNT(index_key) FROM {}index_{}",
        prefix, index
    );
    let mut stmt = conn
        .prepare_cached(&query)
